# Project Rules — Productivity & Accountability App
...
[docs/business_rules.md](docs/business_rules.md) — check it when working on feature-specific logic.
...
'assigned_date': date.toIso8601String().split('T').first,
...
```dart
testWidgets('task card shows completed checkmark', (tester) async {
...
The following Claude Code skills live in `.claude/skills/` (converted from the
former Cursor skills in `.cursor/skills/`) and should be invoked when the task
matches their description:

- `adaptive-layout` — responsive/adaptive layouts across mobile and web
- `architecture-check` — audit code against this file's layer rules
- `design-to-screen` — turn a design into a full Flutter screen
- `design-to-widget` — turn a design into a reusable Flutter widget
- `dto-domain-mapper` — build/update a DTO ↔ domain mapping
- `flutter-feature-generator` — scaffold a brand-new feature folder
- `flutter-ui-builder` — general Flutter UI/widget construction
- `repository-implementation` — abstract + Supabase repository + provider
- `riverpod-controller` — pick and generate the right Riverpod controller/provider
- `riverpod-test-generator` — tests for controllers, repositories, widgets
- `supabase-query-builder` — direct Supabase SDK queries
- `supabase-rpc-helper` — Supabase Postgres RPC calls
